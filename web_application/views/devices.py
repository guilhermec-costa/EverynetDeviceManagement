...
class Device(Session, userMenu):
    ...
    async def get_uplink_message(self, deveui, start_date, end_date, limit):
        start_date = start_date
        end_date = end_date
        limit = limit
        dynamic_url = Session.base_url + f'/history/data.json?access_token={Session.token}&from={start_date}&to={end_date}' \
        f'&limit={limit}&devices={deveui}&types=uplink&duplicate=false'
        async with aiohttp.ClientSession() as session:
            print(dynamic_url)
            async with session.request('GET', dynamic_url, headers=Session.header, data="") as response:
            # while True:
            #     self.messages.prLightPurple('*' * 30)
            #     try:
            #         #start_date = str(input('Type the start date (yyyymmdd) > ')) para input do usuário
            #         start_date = start_date
            #         #end_date = str(input('Type the end date (yyyymmdd) > ')) para input do usuário
            #         end_date = end_date
            ...
            # #deveui = input('Type a deveui > ') para input do usuário
            
            # while True:
            #     try:
            #         #limit = int(input('Messages limit > ')) para input do usupario
            #         limit = limit
            #         break
            #     except Exception as error:
                    # self.validate_option(error)
            ...
            #input("Press enter to continue")
                ...
                return await self._check_request_status(response)
